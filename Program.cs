using CW2_PROJECT_SOLUTION_TEAMC;
using System;
...
class Program
{
    static void Main(string[] args)
    {
        ...
        foreach (var row in productDataList)
        {
            ...
            int stockQuantity = int.Parse(row["StockQuantity"]);
            int categoryId = int.Parse(row["CategoryID"]);

            Product product = new Product(productId, productName, productDescription, price, stockQuantity, categoryId);
